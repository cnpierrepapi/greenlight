...
// The policy watcher (scripts/watch-policies.mjs) records when each source page
// was last fetched and whether it still matched the text the pack was written
// from. Carry that into the bundle so the app can show it. Absent file means
// the watcher has never run, and the UI says nothing rather than guessing.
let watch = null
try {
  const raw = JSON.parse(await readFile(join(PACKS_DIR, 'policy-watch.json'), 'utf8'))
  watch = {
    lastChecked: raw.lastChecked ?? null,
    checkedOk: raw.checkedOk ?? 0,
    total: raw.total ?? packs.length,
    // A source with a pendingHash has moved and nobody has reconciled it yet.
    drifted: Object.entries(raw.sources ?? {})
      .filter(([, s]) => s.pendingHash)
      .map(([id]) => id),
    blocked: Object.entries(raw.sources ?? {})
...
export const POLICY_WATCH: {
  ...
  drifted: string[]
  blocked: string[]
} | null = ${JSON.stringify(watch, null, 2)}
`
